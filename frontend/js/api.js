...
const Api = (() => {
  ...
  return {
    ...
    caixas: {
      ...
      criarAuto:    (data)      => Api.post('/caixas/auto', data),
      proximoCodigo:()          => Api.get('/caixas/proximo-codigo'),
    },

    reparo: {
      prioridades:              ()           => Api.get('/reparo/prioridades'),
      prioridadesTodos:         ()           => Api.get('/reparo/prioridades?todos=true'),
      ...
      vincularSolicitacao:      (id, d)      => Api.post(`/reparo/${id}/vincular-solicitacao`, d),
    },
  };
})();
